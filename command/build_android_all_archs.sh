...
reorganize_structure() {
    ...
    # Find all architecture directories (e.g., arm64-v8a, armeabi-v7a, x86, x86_64)
    local arch_dirs=($(find "$base_path" -maxdepth 1 -type d -name "arm*" -o -name "x86*"))

    for arch_dir in "${arch_dirs[@]}"; do
        # Get the architecture name (e.g., arm64-v8a, x86_64)
...
build() {
    ...
    # Add extra CMake flags if building for x86
    EXTRA_CMAKE_FLAGS=""
    if [ "$arch" = "x86" ]; then
        EXTRA_CMAKE_FLAGS="-DMNN_USE_SSE=false"
    fi
